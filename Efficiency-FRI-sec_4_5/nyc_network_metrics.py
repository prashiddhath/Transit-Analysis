...
DATA_DIR = "input/nyc"
OUTPUT_DIR = "outputs/nyc"

# NYC parameters
NYC_POPULATION = 8_336_817  # 2020 census
NYC_AREA_KM2 = 629.2  # km² (NYC land area excluding Staten Island: 778.2 - 149.0)

# Travel parameters
WALKING_SPEED = 5.0  # km/h
TRANSFER_TIME = 2.0  # minutes
AVG_STATION_SPACING = 0.8  # km (NYC is denser)
AVERAGE_SUBWAY_SPEED_KMH = 28  # NYC subway average speed

# SME time thresholds
SME_TIME_THRESHOLDS = [20, 30, 45, 60, 75, 90]  # minutes
...
def load_nyc_data():
    """Load NYC Subway GTFS data"""
    print("=" * 80)
    print("LOADING NYC SUBWAY DATA")
    print("=" * 80)
    ...
    subway_routes_all = routes[routes['route_type'] == 1].copy()
    
    # Exclude express lines (ending with X) and SI (Staten Island Railway)
    # Express lines: 6X, 7X, FX are variants, not separate lines
    # SI is a different rail mode (not part of subway network)
    exclude_routes = subway_routes_all[
        (subway_routes_all['route_id'].str.endswith('X')) | 
        (subway_routes_all['route_id'] == 'SI')
    ]['route_id'].tolist()
    
    subway_routes = subway_routes_all[~subway_routes_all['route_id'].isin(exclude_routes)].copy()
    
    subway_trip_ids = trips[trips['route_id'].isin(subway_routes['route_id'])]['trip_id']
    subway_trips = trips[trips['trip_id'].isin(subway_trip_ids)].copy()
    subway_stop_times = stop_times[stop_times['trip_id'].isin(subway_trip_ids)].copy()
    
    # Get unique stops used by subway
    subway_stop_ids = subway_stop_times['stop_id'].unique()
    subway_stops = stops[stops['stop_id'].isin(subway_stop_ids)].copy()
    
    print(f"\nTotal routes: {len(routes)} (filtered to {len(subway_routes)} subway lines)")
    print(f"Excluded: {exclude_routes}")
    print(f"Subway lines (route_id): {sorted(subway_routes['route_id'].unique())}") 
    print(f"Subway trips: {len(subway_trips)}")
    print(f"Subway stop times: {len(subway_stop_times)}")
    print(f"Subway stops: {len(subway_stops)}")
...
def identify_unique_stations(stops):
    """
    Identify unique stations by grouping stops with the same name
    NYC-specific fix: Use stop_name as primary grouping key because different
    parent_stations can represent the same physical station complex
    (e.g., Times Sq has parent_stations 127, 725, 902, R16 but they're all one station)
    """
    print("\n" + "-" * 80)
    print("IDENTIFYING UNIQUE STATIONS")
    print("-" * 80)
    
    # Group by stop name (cleaned)
    ...
    for idx, row in stops.iterrows():
        ...
        # Use cleaned name as the station key
        station_key = name
...
def build_network_topology(routes, trips, stop_times, stations_df, stop_to_station, avg_travel_times):
    ...
    # NYC: Use route_id (because route_short_name has duplicates like 'S' for 3 different shuttles)
    routes['route_name'] = routes['route_id']  # Use route_id for NYC
    ...
    # CRITICAL FIX: Add transfer edges from GTFS transfers.txt
    # This connects different line groups that don't share GTFS trips
    print(f"\n  Loading GTFS transfers...")
    try:
        transfers_file = os.path.join(DATA_DIR, "transfers.txt")
        transfers_df = pd.read_csv(transfers_file)
        
        # Load stops to check parent_station grouping
        stops_df_raw = pd.read_csv(os.path.join(DATA_DIR, "stops.txt"))
        stop_parent = {}
        for _, stop in stops_df_raw.iterrows():
            stop_id = stop['stop_id']
            # Use parent_station if available, otherwise use stop name
            if pd.notna(stop.get('parent_station')) and stop.get('parent_station') != '':
                parent = str(stop['parent_station'])
            else:
                parent = stop['stop_name']
                if '(' in parent:
                    parent = parent[:parent.index('(')].strip()
            stop_parent[stop_id] = parent
        ...
        for _, transfer in transfers_df.iterrows():
            ...
            # Check if these stops have DIFFERENT parent stations
            # (transfers within same station complex won't connect different network components)
            from_parent = stop_parent.get(from_stop)
            to_parent = stop_parent.get(to_stop)
            
            if from_parent and to_parent and from_parent != to_parent:
                # Get station IDs for these stops
                from_station = stop_to_station.get(from_stop)
                to_station = stop_to_station.get(to_stop)
                
                if from_station is not None and to_station is not None and from_station != to_station:
                    edge_key = tuple(sorted([from_station, to_station]))
                    
                    if edge_key not in edges:
                        # Get transfer time (in seconds, convert to minutes)
                        transfer_time = transfer.get('min_transfer_time', 300) / 60.0  # Default 5 min
                        
                        edges[edge_key] = {
                            'lines': set(),  # Transfer edges don't belong to specific lines
                            'travel_time': transfer_time
                        }
                        transfer_edges_added += 1
        
        print(f"  Added {transfer_edges_added} inter-station transfer edges from GTFS transfers.txt")
    except FileNotFoundError:
        print(f"  No transfers.txt found - transfer edges not added")
    ...
    return {
        ...
        'all_edges': dict(edges),  # Keep full edges for SME/travel time (now includes transfers)
...
def compute_derrible_kennedy_indicators(G, topology, total_length, num_lines):
    ...
    A = NYC_AREA_KM2
    # Coverage (sigma): station coverage as % of area
    # σ = (# stations × π × R²) / A, where R = 0.5 km coverage radius
    ...
    if nx.is_connected(G):
        ...
        if len(relevant_nodes) > 1:
            ...
            delta_transfers = max_transfers  # Number of transfers (line changes)
            delta_trains = delta_transfers + 1  # Number of trains boarded
        else:
            delta_transfers = 0
            delta_trains = 1  # If no transfers, you still board 1 train
    else:
    ...
    # Calculate for both interpretations:
    ...
    return {
        ...
        'tau': tau_by_trains,  # Use trains interpretation as default
        'rho': rho,
        'delta_transfers': delta_transfers,
        'delta_trains': delta_trains,
        'delta': delta_trains,  # Use trains interpretation as default
        'v': v,
        'v_t': v_t,
        'v_e': v_e,
        'e': e,
...
def compute_fri_baseline(G, topology, total_length, num_lines):
    ...
    print(f"    - By transfers (line changes): τ = {indicators['tau_by_transfers']:.4f} (δ = {indicators['delta_transfers']} transfers)")
    print(f"    - By trains (rides): τ = {indicators['tau_by_trains']:.4f} (δ = {indicators['delta_trains']} trains)")
...
def compute_fri(G_baseline, G_full, topology, stations_df, total_length, num_lines, baseline_bpc):
        ...
        for run in range(RANDOM_SCENARIOS_PER_PROB):
            ...
            if G_disrupted.number_of_nodes() > 0:
                ...
                if G_full_disrupted.number_of_nodes() > 1:
                    ...
                    # Calculate as % of original network (use ALL stations, not just D&K special vertices)
            ...
            scenarios.append({
                ...
                'delta_transfers': indicators.get('delta_transfers', 0),
                'delta_trains': indicators.get('delta_trains', 1)
            })
    ...
    for k in TARGETED_FAILURE_COUNTS:
        ...
        if G_disrupted.number_of_nodes() > 0:
            ...
            bpc_by_transfers = predict_boardings_per_capita(indicators['sigma'], indicators['tau_by_transfers'], indicators['rho'])
            bpc_by_trains = predict_boardings_per_capita(indicators['sigma'], indicators['tau_by_trains'], indicators['rho'])
            performance_ratio_by_transfers = bpc_by_transfers / baseline_bpc if baseline_bpc > 0 else 0
            performance_ratio_by_trains = bpc_by_trains / baseline_bpc if baseline_bpc > 0 else 0
            # Apply fragmentation penalty
            baseline_nodes = len(all_stations)
            remaining_nodes = G_disrupted.number_of_nodes()
            fragmentation_penalty = remaining_nodes / baseline_nodes if baseline_nodes > 0 else 0
            
            performance_ratio_by_transfers *= fragmentation_penalty
            performance_ratio_by_trains *= fragmentation_penalty
            ...
            if G_full_disrupted.number_of_nodes() > 1:
                if nx.is_connected(G_full_disrupted):
                    ...
                    for comp in components:
                        ...
                        if n > 1:
                            reachable_pairs += n * (n - 1) / 2
                # Calculate as % of original network (use ALL stations)
                total_original_pairs = total_stations_full * (total_stations_full - 1) / 2
                reachable_pct = (reachable_pairs / total_original_pairs) * 100
        else:
            performance_ratio_by_transfers = 0
...
def compute_sme(G_full, stations_df, total_length):
    ...
    for time_threshold in SME_TIME_THRESHOLDS:
        ...
        pop_millions = NYC_POPULATION / 1_000_000
        ...
        coverage_fraction = reachable_area / NYC_AREA_KM2 if NYC_AREA_KM2 > 0 else 0
...
def visualize_network_graph(G, stations_df, topology, output_path):
    ...
    # Create 4 different layouts
    layouts = {
        'geographic': gps_pos,
        'kamada_kawai': nx.kamada_kawai_layout(G, scale=2),
        'spring': nx.spring_layout(G, k=1.5, iterations=100, seed=42),
    ...
    layout_titles = {
        ...
        'circular': 'Circular Layout (Optimized)'
    }
    ...
    for layout_name, pos in layouts.items():
        ...
        # Title
        ax.set_title(f'NYC Subway Network — {layout_titles[layout_name]}\n' + 
                    f'{G.number_of_nodes()} Vertices · {G.number_of_edges()} Edges · ' +
                    f'Transfer & Terminal Stations',
        ...
        plt.tight_layout()  # Optimize for paper readability
        
        # Save with appropriate filename
        base_name = output_path.replace('.png', '')
        layout_path = f'{base_name}_{layout_name}.png'
        plt.savefig(layout_path, dpi=300, bbox_inches='tight')
        plt.close()
...
def visualize_sme_analysis(sme_df, output_path):
    ...
    # Add value labels (floor to avoid rounding 99.99% up to 100%)
    for x, y in zip(time_thresholds, reachability_pct):
        import math
        y_display = math.floor(y * 10) / 10  # Floor to 1 decimal place
        ax1.text(x, y + 3, f'{y_display:.1f}%', ha='center', fontsize=10, fontweight='bold')
...
def main():
    ...
    print("\n" + "=" * 80)
    print("NYC SUBWAY NETWORK ANALYSIS")
    print("FRI & SME Metrics Implementation")
    print("=" * 80)
    print(f"Start time: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
    
    # Load NYC data
    routes, trips, stop_times, stops = load_nyc_data()
    ...
    visualize_network_graph(G_fri, stations_df, topology, 
                           os.path.join(OUTPUT_DIR, 'ng_nyc.png'))
    
    # FRI Resilience plots - one for each delta interpretation
    visualize_fri_resilience(scenarios_df, 
                            os.path.join(OUTPUT_DIR, 'fri_transfers_nyc.png'),
                            metric_type='transfers')
    visualize_fri_resilience(scenarios_df, 
                            os.path.join(OUTPUT_DIR, 'fri_trains_nyc.png'),
                            metric_type='trains')
    
    visualize_sme_analysis(sme_df, 
                          os.path.join(OUTPUT_DIR, 'sme_nyc.png'))
    visualize_network_degradation(scenarios_df,
                                  os.path.join(OUTPUT_DIR, 'deg_nyc.png'))
