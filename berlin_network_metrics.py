"""
Berlin U-Bahn Network Analysis
Implements FRI (Functional Resilience Index) and SME (System Level Mobility Efficiency) metrics
...
DATA_DIR = "input/berlin_ubahn_data"
OUTPUT_DIR = "outputs/berlin"  # Directory for all output files and visualizations
TRANSFER_TIME_MINUTES = 4  # Average transfer time in minutes
WALKING_SPEED_KMH = 5  # Walking speed in km/h
AVERAGE_SUBWAY_SPEED_KMH = 32  # Average U-Bahn speed

# Berlin city parameters (approximate)
BERLIN_POPULATION = 3_850_000  # Population of Berlin
...
def load_berlin_data():
    """Load Berlin U-Bahn GTFS data"""
    print("=" * 80)
    print("LOADING BERLIN U-BAHN DATA")
    print("=" * 80)
    
    routes = pd.read_csv(os.path.join(DATA_DIR, "u_routes.csv"))
    trips = pd.read_csv(os.path.join(DATA_DIR, "u_trips.csv"))
    stop_times = pd.read_csv(os.path.join(DATA_DIR, "u_stop_times.csv"))
    stops = pd.read_csv(os.path.join(DATA_DIR, "u_stops.csv"))
...
def identify_unique_stations(stops):
    ...
    for idx, row in stops.iterrows():
        ...
        if pd.notna(row['parent_station']) and row['parent_station'] != '':
...
def build_network_topology(routes, trips, stop_times, stations_df, stop_to_station, avg_travel_times):
    ...
    routes['route_name'] = routes['route_short_name_str']
...
def main():
    print("\n" + "=" * 80)
    print("BERLIN U-BAHN NETWORK ANALYSIS")
    print("FRI & SME Metrics Implementation")
    print("=" * 80)
    
    # Load data
    routes, trips, stop_times, stops = load_berlin_data()
