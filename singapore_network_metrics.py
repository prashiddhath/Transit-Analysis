"""
Singapore MRT Network Analysis (Heavy Rail Only)
...
Excludes LRT feeder lines (BP, SE, SW, PE, PW)
"""
...
DATA_DIR = "input/singapore"
OUTPUT_DIR = "outputs/singapore"
...
def load_singapore_data():
    ...
    print("=" * 80)
    print("LOADING SINGAPORE MRT DATA (Heavy Rail Only)")
    print("=" * 80)
    ...
    # Filter for MRT only (route_type == 1)
    mrt_routes_all = routes[routes['route_type'] == 1].copy()
    
    # EXCLUDE LRT lines (BP, SE, SW, PE, PW) - keep only heavy rail MRT
    LRT_LINES = ['BP', 'SE', 'SW', 'PE', 'PW']
    mrt_routes = mrt_routes_all[~mrt_routes_all['route_short_name'].isin(LRT_LINES)].copy()
    ...
    print(f"\nTotal routes: {len(routes)} (filtered to {len(mrt_routes)} MRT heavy rail lines)")
    print(f"Excluded LRT lines: {LRT_LINES}")
    print(f"MRT lines: {sorted(mrt_routes['route_short_name'].unique())}")
    print(f"MRT trips: {len(mrt_trips)}")
    print(f"MRT stop times: {len(mrt_stop_times)}")
...
def visualize_network_graph(G, stations_df, topology, output_path):
    ...
    for layout_name, pos in layouts.items():
        ...
        # Title
        ax.set_title(f'Singapore MRT Network — {layout_titles[layout_name]}\n' + 
...
def main():
    ...
    visualize_network_graph(G_fri, stations_df, topology, 
                           os.path.join(OUTPUT_DIR, 'ng_singapore.png'))
    
    # FRI Resilience plots - one for each delta interpretation
    visualize_fri_resilience(scenarios_df, 
                            os.path.join(OUTPUT_DIR, 'fri_transfers_singapore.png'),
                            metric_type='transfers')
    visualize_fri_resilience(scenarios_df, 
                            os.path.join(OUTPUT_DIR, 'fri_trains_singapore.png'),
                            metric_type='trains')
    
    visualize_sme_analysis(sme_df, 
                          os.path.join(OUTPUT_DIR, 'sme_singapore.png'))
    visualize_network_degradation(scenarios_df,
                                  os.path.join(OUTPUT_DIR, 'deg_singapore.png'))
